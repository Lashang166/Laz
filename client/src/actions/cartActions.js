...
export default {
    ...
    checkout: (cartItems, address, payment, shipper, totalPrice) => async (dispatch) => {
        const items= []
        ...
        console.log(items);
        dispatch({ type: "CART_ADD_REQUEST" })
        const { data } = await axios.post("/api/order/add", { items, address, payment, shipper , totalPrice })
        console.log(data);

        dispatch({ type: "CART_CHECK_OUT", payload: items, address, payment, shipper, totalPrice})
    }
}